...
def resource_path(relative_path):
    """Gets the absolute path to the resource, works in .exe too"""
...
class IPKEditor:
    def __init__(self, root):
        ...
        # GUI elements
        self.label = tk.Label(root, text="Select IPK file for extraction")
        self.label.pack(pady=10)

        self.select_button = tk.Button(root, text="Select IPK", command=self.select_ipk)
        self.select_button.pack(pady=5)

        self.extract_button = tk.Button(root, text="Extract", command=self.extract_ipk, state=tk.DISABLED)
        self.extract_button.pack(pady=5)

        self.pack_button = tk.Button(root, text="Pack IPK", command=self.pack_ipk, state=tk.DISABLED)
    ...
    def select_ipk(self):
        ...
        if self.ipk_file:
            ...
            self.status_label.config(text=f"Selected file: {os.path.basename(self.ipk_file)}")
        else:
            self.extract_button.config(state=tk.DISABLED)
            self.status_label.config(text="File not selected")

    def extract_ipk(self):
        if not self.ipk_file:
            messagebox.showerror("Error", "First select the IPK file")
            return
        ...
        if self.is_gzip_file(self.ipk_file):
            ...
            self.status_label.config(text=f"GZIP decompressed to {temp_ipk}")
        ...
        try:
            if self.is_ar_archive(temp_ipk):
                ...
                self.status_label.config(text="Not an ar-archive, processing as tar...")
            ...
            self.status_label.config(text=f"Extracted to {self.extract_dir}. Edit the files and press 'Pack IPK'.")
            os.startfile(self.extract_dir)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to extract IPK: {str(e)}")
            self.status_label.config(text=f"Error during extraction: {str(e)}")

    def pack_ipk(self):
        if not self.extract_dir:
            messagebox.showerror("Error", "First extract the IPK file")
            return
        ...
        try:
            # Creating .tar.gz for folders control, data, debian (if present)
            ...
            for sub in sub_dirs:
                ...
                self.status_label.config(text=f"Created {tar_path}")

            # Creating debian-binary
            ...
            if not os.path.exists(debian_binary):
                with open(debian_binary, 'w') as f:
                    f.write('2.0\n')
                self.status_label.config(text=f"Created {debian_binary}")

            # Collecting files for the archive
            ar_files = [debian_binary] + tar_files
            self.status_label.config(text=f"Files to pack: {', '.join(os.path.basename(f) for f in ar_files)}")

            # Creating IPK in a temporary folder to avoid locks on the original directory
            with tempfile.TemporaryDirectory() as temp_dir:
                temp_ipk = os.path.join(temp_dir, f"{base_name}")  # Temporary file without extension
                ...
                # Now copying from temporary folder to final location
                time.sleep(0.5)  # More delay
                if os.path.exists(final_ipk):
                    os.remove(final_ipk)
                shutil.copy2(temp_ipk, final_ipk)
                self.status_label.config(text=f"File copied to {final_ipk}")

                if os.path.exists(self.extract_dir):
                    shutil.rmtree(self.extract_dir)
            
            self.status_label.config(text=f"IPK packed: {final_ipk}")
            messagebox.showinfo("Success", f"IPK successfully packed: {final_ipk}")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to pack IPK: {str(e)}")
            self.status_label.config(text=f"Error during packing: {str(e)}")
